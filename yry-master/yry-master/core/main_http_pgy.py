...
@app.route('/face_point_p', methods=['POST'])
def face_point():
    data_dict = request.get_json()
    is_exp = data_dict.get("is_exp")
    # print(data_dict)
    # w = request.form.get("is_exp")
    is_exp = int(is_exp)
    face_data = data_dict.get("face_data")
    account_body_id = data_dict.get("account_body_id")
    account_body_id = int(account_body_id)
    ...
    if is_exp == 0:
        conn = pymysql.connect(host='192.168.20.30', port=3306, user='tbroot', password='tbmima7800', db='pgy')
        cursor = conn.cursor()
    ...
    if is_exp == 1:
        ...
        if account_body_id:
            if sex == 2:
                ...
                core.face_merge_nan_g(src_img=model_image_nan,
                                      src_points=src_points,
                                      dst_img=source_image1_draw,
                                      out_img=face_image_output,
                                      dst_points=dst_points,
...
def face_point1():  # 接口主体部分，运行内容
    data_dict = request.get_json()
    face_type = data_dict.get("face_type")
    is_exp = data_dict.get("is_exp")
    is_exp = int(is_exp)
    # is_exp=int(is_exp)
    # token = request.form.get("token")
